...
class Database:
    ...
    async def init(self):
        ...
        async with aiosqlite.connect(self.path) as db:
            ...
            await db.execute("""CREATE TABLE IF NOT EXISTS user_settings (telegram_user_id INTEGER PRIMARY KEY, model TEXT, provider TEXT, updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)""")
            cols = await (await db.execute("PRAGMA table_info(user_settings)")).fetchall()
            if not any(c[1] == "provider" for c in cols):
